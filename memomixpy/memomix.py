...
class MemoMix:
    ...
    def set_person_ids(self, person_ids: set):
        ...
        """
        assert len(person_ids) <= sum(self.group_sizes.values()), 'The groups cannot contain all the persons.'
        self.person_ids = person_ids
    ...
    def set_group_sizes(self, group_sizes: dict):
        ...
        """
        assert len(self.person_ids) <= sum(group_sizes.values()), 'The groups cannot contain all the persons.'
        assert all([size >= 1 for size in group_sizes.values()]), 'At least one group has a negative or null size.'
        self.group_sizes = group_sizes

    def set_constraints(self, together_constraints: list, apart_constraints: list):
        """
        Change the sets of 'together' and 'apart' constraints.
    ...
    def initialize_entry(self, person_occurrences_map, group_occurrences_map):
        """
        ...
        for constraint in self.together_constraints:
            person_ids = constraint['personIds']
            if 'inGroup' in constraint:
                group_id = constraint['inGroup']
            else:
        ...
        for constraint in self.apart_constraints:
            person_ids = constraint['personIds']
            ...
            for person_id in person_ids:
                ...
                if priority_group_ids:
                    group_id = random.choice(list(priority_group_ids))
    ...
    def get_new_entry(self):
        ...
        :return: a new entry
        """
        # CAVEAT: This is a heuristic algorithm, not an optimal one.
        # Finding the group configuration that has the lowest redundancy
